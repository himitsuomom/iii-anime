...
| [`catalog`](#iii-worker-catalog) | Browse the curated catalog of first-party iii workers, grouped by category, each with the `iii worker add` command that installs it. This is an offline index of the workers baked into this engine build; browse the full community registry at https://workers.iii.dev |
...
Browse the curated catalog of first-party iii workers, grouped by category, each with the `iii worker add` command that installs it. This is an offline index of the workers baked into this engine build; browse the full community registry at https://workers.iii.dev

```text
iii worker catalog [OPTIONS] [QUERY]
```

| Argument | Description |
| -------- | ----------- |
| `[QUERY]` | Case-insensitive filter matched against worker name, category, and description. Omit to show every worker |

| Option | Description |
| ------ | ----------- |
| `--names-only` | Print only matching worker names, one per line (script-friendly, e.g. `iii worker catalog messaging --names-only`) |
