...
/**
 * Register an HTTP-triggered function in one call (same pattern as the SDK
 * example). Function ids follow the `api::<method>::<path>` convention.
 */
...
export const useApi = <TBody = any>(
  config: { api_path: string; http_method: string; description?: string; metadata?: Record<string, unknown> },
