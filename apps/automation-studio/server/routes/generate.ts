...
import { buildTemplateDescription } from '../lib/offline.ts'
...
generateRoute.post('/', async (c) => {
  ...
  // API-less fallback: no key → deterministic template generation (HTTP 200).
  const client = getClient()
  if (!client) {
    const result = buildTemplateDescription({ ...body, productName })
    return c.json({ result, source: 'template' })
  }
